...
Start the component workbench, built with Portfolio, alongside the app:
...
- `http://localhost:8080/ui-workbench.html` for the workbench

For workbench-only iteration:

```bash
npm run portfolio
```

Workbench source lives under `/hyperopen/portfolio/hyperopen/workbench/`.

- Scenes live in `/hyperopen/portfolio/hyperopen/workbench/scenes/**`
- Shared builders/helpers live in `/hyperopen/portfolio/hyperopen/workbench/support/**`
- Scene files use the `*_scenes.cljs` naming convention
- Scene namespaces end in `-scenes`

When adding new scenes, prefer real view functions from `/hyperopen/src/hyperopen/views/**`, keep new workbench-only helpers in `/hyperopen/portfolio/hyperopen/workbench/support/**`, and avoid requiring workbench namespaces from production code.
