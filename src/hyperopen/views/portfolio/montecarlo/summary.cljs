(ns hyperopen.views.portfolio.montecarlo.summary
  ...
  percentile table for the Monte Carlo tab."
  (:require [hyperopen.views.portfolio.montecarlo.format :as fmt]))

(defn- prob-row
  [{:keys [label kind value sub]}]
  (let [bust? (= kind :bust)]
    [:div {:class ["mc-prob"]
           :data-role (str "portfolio-monte-carlo-prob-" (name kind))}
...
(defn prob-card
  ...
  (let [{:keys [sims bust goal]} controls
    ...
    [:div {:class ["mc-card" "mc-prob-card"]
           :data-role "portfolio-monte-carlo-probabilities"}
...
(defn percentile-table
  [{:keys [result controls live-equity]}]
  (let [term (:terminal result)
    ...
    [:div {:class ["mc-card" "mc-card-pad"]
           :data-role "portfolio-monte-carlo-terminal-table"}
