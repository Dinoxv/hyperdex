...
(defn- toast-card
  ...
  (case (:toast-surface toast)
    ...
    :order-submitted (status-toast-card toast "Dismiss order submitted notification")
    :order-canceled (status-toast-card toast "Dismiss order canceled notification")
    (generic-toast-card toast)))
