(ns hyperopen.views.trade.order-form-intent-adapter
  (:require [hyperopen.views.trade.order-form-commands :as commands]))
...
(def ^:private placeholder-by-token
  {commands/event-target-value [:event.target/value]
   commands/event-target-checked [:event.target/checked]
   commands/event-key [:event/key]})

(defn- resolve-command-arg [arg]
  (if-let [placeholder (get placeholder-by-token arg)]
    placeholder
    arg))

(defn command->actions
  [{:keys [command-id args] :as command}]
