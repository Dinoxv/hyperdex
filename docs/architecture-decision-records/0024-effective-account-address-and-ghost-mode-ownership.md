# ADR 0024: Effective Account Address and Ghost Mode Ownership Split
...
Trade.xyz-style Ghost Mode requires a read-only spectating identity that can differ from the connected wallet owner. Reusing `[:wallet :address]` for both concerns makes that unsafe, because spectating could accidentally leak into signing or agent-session ownership paths.
...
3. Introduce Ghost Mode read identity (`[:account-context :ghost-mode :address]`) with explicit active state.
4. Define `effective-account-address` as:
   - ghost address when Ghost Mode is active,
   - otherwise owner address.
5. Read-side subscription/fetch/projection ownership must use `effective-account-address`.
6. Mutation eligibility must be centralized as `mutations-allowed?` and default to false during active Ghost Mode.

## Consequences

- Ghost Mode can change account read context without changing signer ownership.
...
- Startup state restoration for Ghost Mode preference/watchlist:
