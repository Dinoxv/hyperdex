...
- Observation: the older deferred namespace-size audit is stale for this worktree, but the live boundary-debt baseline has continued shrinking while the size-debt baseline stayed flat.
  Evidence: `/hyperopen/dev/namespace_size_exceptions.edn` still carries 85 entries generated from current `src/**/*.cljs` and `test/**/*.cljs` files above 500 lines, while `/hyperopen/dev/namespace_boundary_exceptions.edn` dropped from the original 9 non-view `hyperopen.views.*` import edges to 3 after `DIP-01` and `DIP-02`.
