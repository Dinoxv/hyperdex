...
- [x] (2026-04-02 01:16Z) Re-ran targeted `/trade` Playwright smoke, `npm test`, `npm run test:websocket`, `npm run check`, and three follow-up local startup profiles, with the latest profile improving to `blockingTimeProxyMs 182` and `maxSingleBlockingTaskMs 232`.
- [x] (2026-04-02 02:08Z) Changed `browser:profile:trade-startup` to rebuild release assets before profiling and added `browser:profile:trade-startup:cached` for post-build reruns against the same asset set.
- [x] (2026-04-02 02:08Z) Lazy-initialized the chart navigation overlay so cold startup binds only container listeners and defers the overlay DOM subtree plus document keyboard listener until the activation zone is actually hit.
- [x] (2026-04-02 02:09Z) Re-ran targeted `/trade` Playwright smoke, `npm test`, `npm run test:websocket`, `npm run check`, and two fresh release-backed startup profiles at `tmp/browser-inspection/trade-startup-profile-2026-04-01T22-08-11-569Z-c86ac31f/` and `tmp/browser-inspection/trade-startup-profile-2026-04-01T22-08-33-496Z-8de4bdac/`, recording `48/98` and `43/93` blocking/max-long-task respectively.
...
- Observation: deferring chart decorations alone was not enough to move the local startup trace, but overlay subscription coalescing and narrower pane subscriptions produced the best local profile of the initiative so far.
  Evidence: the first chart-core cleanup profile at `tmp/browser-inspection/trade-startup-profile-2026-04-01T21-06-41-081Z-90058123/profile.json` recorded `201/251`, the first overlay-focused profile at `tmp/browser-inspection/trade-startup-profile-2026-04-01T21-14-29-777Z-365cbadb/profile.json` recorded `198/248`, and the second overlay-focused profile at `tmp/browser-inspection/trade-startup-profile-2026-04-01T21-15-32-683Z-a270507a/profile.json` recorded `blockingTimeProxyMs 182`, `maxSingleBlockingTaskMs 232`, `tradeRootVisibleMs 391.8`, and `orderFormVisibleMs 394.6`.

- Observation: the local startup profiler had been serving `out/release-public` without rebuilding it first, which meant earlier keep/reject calls could be measuring stale release assets instead of the current source tree.
  Evidence: before 2026-04-02 02:08Z, `package.json` defined `browser:profile:trade-startup` as `node tools/browser-inspection/src/trade_startup_profile.mjs` while the profiler served `PLAYWRIGHT_STATIC_ROOT=out/release-public`; after changing the script to run `npm run build` first, the next two local startup profiles dropped to `48/98` and `43/93`.

- Observation: deferring the chart navigation overlay DOM subtree until first activation materially reduced the cold startup work on freshly built release assets.
  Evidence: `/hyperopen/src/hyperopen/views/trading_chart/utils/chart_interop/chart_navigation_overlay.cljs` now waits until hover or focus activation to create the overlay root and document keyboard listener, `test/hyperopen/views/trading_chart/utils/chart_interop/chart_navigation_overlay_test.cljs` now asserts that cold sync leaves `:root`, `:panel`, and the document `keydown` listener unset, and the fresh release-backed profiles at `tmp/browser-inspection/trade-startup-profile-2026-04-01T22-08-11-569Z-c86ac31f/profile.json` and `tmp/browser-inspection/trade-startup-profile-2026-04-01T22-08-33-496Z-8de4bdac/profile.json` recorded `blockingTimeProxyMs` of `48` and `43`.
...
- Decision: keep chart-overlay repaint batching and the narrower volume-overlay subscription surface even though the first chart-core deferral pass alone was mostly neutral.
  Rationale: the overlay-focused follow-up preserved visible behavior under deterministic tests and improved the local startup profile to `182/232`, which makes it a justifiable Milestone 2 cut instead of speculative cleanup.
  Date/Author: 2026-04-02 / Codex

- Decision: make the default trade-startup profiler rebuild release assets before serving `out/release-public`, and keep a separate cached-profile script for fast reruns against the same asset set.
  Rationale: local performance decisions are only trustworthy when the measured assets reflect the current source tree; the cached variant preserves fast back-to-back comparisons once a fresh release build exists.
  Date/Author: 2026-04-02 / Codex

- Decision: defer chart-navigation overlay DOM creation and document keyboard listeners until the overlay activation zone is actually hit.
