...
- [x] (2026-03-17 16:42Z) Re-ran Milestone 5 boundary discovery after the JS wave and confirmed there is no further bounded in-repo cache patch on the current shipped path: `npm run build` still uses `shadow-cljs release app`, parser-time CSS still comes from tracked `/hyperopen/resources/public/index.html` via `/css/main.css`, and safe CSS fingerprinting now requires a broader release-HTML generation or separate artifact-root redesign instead of another small cache tweak.
...
- Observation: CSS fingerprinting is not a narrow follow-on to the JS wave because the shipped build still relies on a tracked shared app entrypoint for parser-time stylesheet loading.
  Evidence: `/hyperopen/resources/public/index.html` still hardcodes `<link rel="stylesheet" href="/css/main.css" />`, `/hyperopen/.gitignore` ignores generated CSS and JS outputs but not `index.html`, and `/hyperopen/test/hyperopen/views/typography_scale_test.cljs` plus dev-serving workflows still read that tracked file directly.
...
- Decision: Do not broaden the existing service worker to cache local JS/CSS/chunk assets while the release build still uses stable asset URLs.
...
- Decision: Stop Milestone 5 implementation after the JS wave until the next release-entry redesign is taken on explicitly.
  Rationale: a further in-repo step now requires generated or release-only HTML ownership so CSS can stay parser-time while using fingerprinted URLs. Trying to force CSS versioning into the current tracked shared `index.html` would either delay style application through runtime lookup or bleed release state into the dev-served source entrypoint.
  Date/Author: 2026-03-17 / Codex
...
The first Milestone 5 investigation clarified the safe boundary, and the first in-repo follow-up has now landed its smallest safe slice. The repo already owns a narrow icon-only service worker and can keep that behavior. The release build now emits hashed JS module URLs and a manifest-driven entrypoint bootstrap, which removes the prior mixed-generation risk for `main` and async browser chunks. The remaining gap is explicit: `/css/main.css` still uses a stable URL, and deployment-owned cache headers are still outside this repo. Just as importantly, the current shipped build path still treats `/hyperopen/resources/public/index.html` as a tracked shared source file for both dev and release. That means Milestone 5 remains open even after this first `hyperopen-8jtk` cut; the next safe product move is either deployment-owned revalidation or an explicit release-entry redesign that generates release HTML/CSS separately before any attempt to cache local CSS or declare full immutable-asset coverage.
