...
- [x] (2026-03-26 22:47 EDT) Replaced `/hyperopen/tools/formal/lean/Hyperopen/Formal/OrderRequest/Advanced.lean` with a modeled advanced-order surface for scale ladders and TWAP requests, promoted `order-request-advanced` to a generated-source surface in `/hyperopen/tools/formal/core.clj`, regenerated `/hyperopen/test/hyperopen/formal/order_request_advanced_vectors.cljs`, and updated `/hyperopen/test/hyperopen/api/gateway/orders/commands_test.cljs` so raw-builder vectors may still assert legal wire shapes when the builder emits a request.
- [x] (2026-03-26 22:47 EDT) Patched the advanced scale serializer after review found that the first version failed closed on repeating-decimal price steps such as `start=100`, `end=99`, `count=4`. The model now keeps exact arithmetic internally but rounds non-terminating ladder prices at the serialization boundary, and the generated advanced vectors now pin the repeating-step case explicitly.
- [x] (2026-03-27 01:07 EDT) Refreshed `coverage/lcov.info` with `npm run coverage`, added focused mutation-coverage tests in `/hyperopen/test/hyperopen/api/gateway/orders/commands_test.cljs`, `/hyperopen/test/hyperopen/state/trading/market_summary_test.cljs`, and `/hyperopen/test/hyperopen/state/trading/validation_and_scale_test.cljs`, and recorded the current scan evidence: `/hyperopen/src/hyperopen/vaults/domain/transfer_policy.cljs` still has a full `22/22` killed run, `/hyperopen/src/hyperopen/api/gateway/orders/commands.cljs` now scans as `30/31` covered with only `L34 if -> if-not` uncovered, `/hyperopen/src/hyperopen/domain/trading/market.cljs` now scans as `110/110` covered, and `/hyperopen/src/hyperopen/domain/trading/core.cljs` now scans as `91/94` covered with only the `scale-weights` raw-negative fallback lines uncovered.
- [x] (2026-03-27 02:09 EDT) Closed the live mutation follow-up on the two primary order proof kernels. `/hyperopen/src/hyperopen/api/gateway/orders/commands.cljs` now has explicit mutation evidence across all 30 covered sites, including dedicated reruns for the leverage/margin branches and new boundary tests for string `onlyIsolated` flags and minimum positive leverage. `/hyperopen/src/hyperopen/domain/trading/market.cljs` now has targeted proof-surface mutation evidence for the canonical formatter helpers (`8/8` killed on lines `155-175`) and the public price/size canonicalization path (`5/5` killed on lines `176-202`). `/hyperopen/src/hyperopen/domain/trading/core.cljs` now has targeted mutation evidence for the TWAP and size-flooring range (`18/18` killed on lines `156-208`), while the remaining scale-ladder work is narrowed to `scale-weights` equivalence/uncovered fallback behavior rather than live regressions.
...
- Observation: the mutation tool depends on `coverage/lcov.info`, not on a plain `npm test` run.
  Evidence: the new market/core-focused tests did not change mutation scan coverage until `npm run coverage` regenerated `coverage/lcov.info` on 2026-03-27.

- Observation: `tools/mutate.clj` can leave stale backups and orphaned Babashka processes when a `commands.cljs` mutation run is interrupted or times out, and those orphans can continue mutating the working tree after the parent command appears done.
  Evidence: multiple orphaned `bb tools/mutate.clj --module src/hyperopen/api/gateway/orders/commands.cljs ...` processes had to be killed on 2026-03-27, and stale backups under `/hyperopen/target/mutation/backups/src/hyperopen/api/gateway/orders/commands.cljs.bak` repeatedly reintroduced mutations such as `max 1 -> max 0` and `when -> when-not`.

- Observation: the remaining stubborn `scale-weights` mutation sites are in algebraically special branches, not in the money-moving TWAP/flooring path.
  Evidence: after the final reruns, `/hyperopen/src/hyperopen/domain/trading/core.cljs` killed every targeted mutant on lines `156-208`, but the unfinished `261-280` pass only continued surfacing `n = 1` and normalized-weight setup mutations while the full repo gates stayed green. The scan still reports the only uncovered sites at lines `273` and `275`, which are the defensive raw-negative fallback inside `scale-weights`.
...
- Observation: exact-arithmetic scale modeling still needs an explicit serialization policy at the boundary.
  Evidence: the first advanced-order implementation failed closed on repeating-decimal ladder prices because `/hyperopen/tools/formal/lean/Hyperopen/Formal/OrderRequest/Advanced.lean` initially only emitted finite decimal strings. A review against `/hyperopen/src/hyperopen/domain/trading/core.cljs` and `/hyperopen/src/hyperopen/api/gateway/orders/commands.cljs` exposed the `start=100`, `end=99`, `count=4` counterexample, and the plan now records the patched rounded-boundary serializer plus a generated vector for that case.

- Observation: mutation close-out depends on fresh coverage and is still the only unfinished acceptance item.
  Evidence: `bb tools/mutate.clj --module src/hyperopen/vaults/domain/transfer_policy.cljs --suite test --mutate-all` failed closed until `npm run coverage` produced `coverage/lcov.info`, and the full four-module mutation sweep was not completed in this session after implementation defects were discovered and fixed mid-run.
...
The fourth implementation slice completed the advanced-order surface and exposed one useful truth immediately: the model needed an explicit boundary serializer for non-terminating scale prices even though the internal arithmetic stayed exact. At this point all three formal surfaces are modeled and generated, and the only remaining close-out work is mutation evidence plus any follow-up it uncovers.

If the remaining mutation milestone succeeds, overall complexity should still go down. Hyperopen will trade a modest amount of optional proof-tooling complexity for a much clearer and reviewable definition of what vault and order actions are legal to emit.
...
The current repo state matters. `/hyperopen/tools/formal/lean/Hyperopen/Formal/Common.lean` now owns command parsing, surface metadata, manifest helpers, and deterministic Clojure rendering helpers. `/hyperopen/tools/formal/lean/Hyperopen/Formal/VaultTransfer.lean`, `/hyperopen/tools/formal/lean/Hyperopen/Formal/OrderRequest/Standard.lean`, and `/hyperopen/tools/formal/lean/Hyperopen/Formal/OrderRequest/Advanced.lean` now all emit generated proof-facing namespaces under `/hyperopen/target/formal/**`. `/hyperopen/tools/formal/core.clj` verifies and syncs all three modeled generated namespaces in addition to manifests.
...
Plan update note (2026-03-26 22:47 EDT): Completed the advanced-order milestone by promoting `order-request-advanced` to a modeled generated-source surface and adding a generated repeating-decimal scale case after review exposed the first serializer mismatch. The only remaining open acceptance item is the targeted mutation close-out across the four claimed kernels.
