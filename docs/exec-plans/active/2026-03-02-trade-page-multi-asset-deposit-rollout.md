...
- [x] (2026-03-02 22:14Z) Completed Milestone 5 (`2Z`) HyperUnit address-flow enablement by adding `2z` Solana source-chain metadata to the shared address-generation path in `/hyperopen/src/hyperopen/funding/actions.cljs`.
- [x] (2026-03-02 22:15Z) Added `2Z`-specific request/response coverage in `/hyperopen/test/hyperopen/funding/actions_test.cljs` and `/hyperopen/test/hyperopen/funding/effects_test.cljs`.
- [x] (2026-03-02 22:18Z) Re-ran required validation gates after Milestone 5: `npm run check`, `npm test`, `npm run test:websocket` (all pass; existing unrelated test compile warnings remain).
- [x] (2026-03-02 22:28Z) Completed Milestone 6 (`BONK`) HyperUnit address-flow enablement by adding `bonk` Solana source-chain metadata to the shared address-generation path in `/hyperopen/src/hyperopen/funding/actions.cljs`.
- [x] (2026-03-02 22:29Z) Added `BONK`-specific request/response coverage in `/hyperopen/test/hyperopen/funding/actions_test.cljs` and `/hyperopen/test/hyperopen/funding/effects_test.cljs`.
- [x] (2026-03-02 22:31Z) Re-ran required validation gates after Milestone 6: `npm run check`, `npm test`, `npm run test:websocket` (all pass; existing unrelated test compile warnings remain).
- [x] (2026-03-02 22:41Z) Completed Milestone 7 (`ENA`) HyperUnit address-flow enablement by adding `ena` Ethereum source-chain metadata to the shared address-generation path in `/hyperopen/src/hyperopen/funding/actions.cljs`.
- [x] (2026-03-02 22:42Z) Added `ENA`-specific request/response coverage in `/hyperopen/test/hyperopen/funding/actions_test.cljs` and `/hyperopen/test/hyperopen/funding/effects_test.cljs`.
- [x] (2026-03-02 22:44Z) Re-ran required validation gates after Milestone 7: `npm run check`, `npm test`, `npm run test:websocket` (all pass; existing unrelated test compile warnings remain).
- [x] (2026-03-02 22:54Z) Completed Milestone 8 (`FARTCOIN`) HyperUnit address-flow enablement by adding `fart` Solana source-chain metadata to the shared address-generation path in `/hyperopen/src/hyperopen/funding/actions.cljs`.
- [x] (2026-03-02 22:55Z) Added `FARTCOIN`-specific request/response coverage in `/hyperopen/test/hyperopen/funding/actions_test.cljs` and `/hyperopen/test/hyperopen/funding/effects_test.cljs`.
- [x] (2026-03-02 22:57Z) Re-ran required validation gates after Milestone 8: `npm run check`, `npm test`, `npm run test:websocket` (all pass; existing unrelated test compile warnings remain).
- [x] (2026-03-02 23:08Z) Completed Milestone 9 (`MON`) HyperUnit address-flow enablement by adding `mon` Monad source-chain metadata to the shared address-generation path in `/hyperopen/src/hyperopen/funding/actions.cljs`.
- [x] (2026-03-02 23:09Z) Added `MON`-specific request/response coverage in `/hyperopen/test/hyperopen/funding/actions_test.cljs` and `/hyperopen/test/hyperopen/funding/effects_test.cljs`.
- [x] (2026-03-02 23:11Z) Re-ran required validation gates after Milestone 9: `npm run check`, `npm test`, `npm run test:websocket` (all pass; existing unrelated test compile warnings remain).
- [x] (2026-03-02 23:22Z) Completed Milestone 10 (`PUMP`) HyperUnit address-flow enablement by adding `pump` Solana source-chain metadata to the shared address-generation path in `/hyperopen/src/hyperopen/funding/actions.cljs`.
- [x] (2026-03-02 23:23Z) Added `PUMP`-specific request/response coverage in `/hyperopen/test/hyperopen/funding/actions_test.cljs` and `/hyperopen/test/hyperopen/funding/effects_test.cljs`.
- [x] (2026-03-02 23:25Z) Re-ran required validation gates after Milestone 10: `npm run check`, `npm test`, `npm run test:websocket` (all pass; existing unrelated test compile warnings remain).
- [x] (2026-03-02 23:36Z) Completed Milestone 11 (`SPX`) HyperUnit address-flow enablement by adding `spxs` Solana source-chain metadata to the shared address-generation path in `/hyperopen/src/hyperopen/funding/actions.cljs`.
- [x] (2026-03-02 23:37Z) Added `SPX`-specific request/response coverage in `/hyperopen/test/hyperopen/funding/actions_test.cljs` and `/hyperopen/test/hyperopen/funding/effects_test.cljs`.
- [x] (2026-03-02 23:39Z) Re-ran required validation gates after Milestone 11: `npm run check`, `npm test`, `npm run test:websocket` (all pass; existing unrelated test compile warnings remain).
- [ ] Implement remaining per-coin milestones one by one with tests and manual verification (XPL, plus any additional assets discovered in Milestone 0 such as USDH).
...
Planning + discovery + Milestone 1 (`USDT`) + Milestone 2 (`BTC`) + Milestone 3 (`ETH`) + Milestone 4 (`SOL`) + Milestone 5 (`2Z`) + Milestone 6 (`BONK`) + Milestone 7 (`ENA`) + Milestone 8 (`FARTCOIN`) + Milestone 9 (`MON`) + Milestone 10 (`PUMP`) + Milestone 11 (`SPX`) completed.
...
- Implemented fourth HyperUnit address-flow coin (`2Z`) by enabling Solana source-chain metadata for the `2z` asset in the shared address-generation request handling.
- Implemented fifth HyperUnit address-flow coin (`BONK`) by enabling Solana source-chain metadata for the `bonk` asset in the shared address-generation request handling.
- Implemented sixth HyperUnit address-flow coin (`ENA`) by enabling Ethereum source-chain metadata for the `ena` asset in the shared address-generation request handling.
- Implemented seventh HyperUnit address-flow coin (`FARTCOIN`) by enabling Solana source-chain metadata for the `fart` asset in the shared address-generation request handling.
- Implemented eighth HyperUnit address-flow coin (`MON`) by enabling Monad source-chain metadata for the `mon` asset in the shared address-generation request handling.
...
Not yet achieved:
- Remaining per-coin route/address flows (`XPL`, `USDH`) are not implemented yet.
...
Revision note (2026-03-02): Completed Milestone 5 (`2Z`) by enabling Solana HyperUnit address generation for the `2z` asset in the shared address-flow pipeline, adding `2Z` coverage tests, and re-running required validation gates.
Revision note (2026-03-02): Completed Milestone 6 (`BONK`) by enabling Solana HyperUnit address generation for the `bonk` asset in the shared address-flow pipeline, adding `BONK` coverage tests, and re-running required validation gates.
Revision note (2026-03-02): Completed Milestone 7 (`ENA`) by enabling Ethereum HyperUnit address generation for the `ena` asset in the shared address-flow pipeline, adding `ENA` coverage tests, and re-running required validation gates.
Revision note (2026-03-02): Completed Milestone 8 (`FARTCOIN`) by enabling Solana HyperUnit address generation for the `fart` asset in the shared address-flow pipeline, adding `FARTCOIN` coverage tests, and re-running required validation gates.
Revision note (2026-03-02): Completed Milestone 9 (`MON`) by enabling Monad HyperUnit address generation for the `mon` asset in the shared address-flow pipeline, adding `MON` coverage tests, and re-running required validation gates.
