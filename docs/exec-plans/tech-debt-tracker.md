...
- Debt summary: Non-view imports from `hyperopen.views.*` are now enforced through `/hyperopen/dev/namespace_boundary_exceptions.edn`, but nine temporary exceptions remain for bootstrap bridges and `DIP-01` / `DIP-02` backlog items.
  Owner team: Platform
  ...
  Retirement criteria: `DIP-01` must retire the `chart.hover` and `account-info.sort-kernel` exceptions, `DIP-02` must retire the `views.vaults.vm` and `portfolio.vm.metrics-bridge` exceptions, and the remaining bootstrap / console-preload bridges need either extraction or an explicit permanent owner before their `:retire-by` dates.
