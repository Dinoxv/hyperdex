...
(deftest enable-agent-trading-action-emits-approving-before-effect-test
  (let [state {:wallet {:connected? true
        ...
        effects (wallet-actions/enable-agent-trading-action state identity identity)
...
(deftest unlock-agent-trading-action-emits-unlocking-projection-before-effect-test
  (let [state {:wallet {:connected? true
                        :address "0xabc"
                        :agent {:status :locked}}}
        effects (wallet-actions/unlock-agent-trading-action state)
        immediate (first effects)
        io-effect (second effects)]
    (is (= :effects/save-many (first immediate)))
    (is (= [[:wallet :agent :status] :unlocking]
           (-> immediate second first)))
    (is (= [[:wallet :agent :error] nil]
           (-> immediate second second)))
...
(deftest actions-accept-plain-js-normalizers-test
  (let [normalize-js (js/Function. "value"
        ...
        normalize-protection-js (js/Function. "value"
                                              "return (value === 'PASSKEY') ? 'passkey-js' : 'plain-js';")
    ...
    (is (= [[:effects/save-many [[[:wallet :agent :status] :approving]
            ...
            [:effects/enable-agent-trading {:storage-mode "session-js"
                                            :local-protection-mode "passkey-js"}]]
           (wallet-actions/enable-agent-trading-action connected-state
                                                       normalize-js
                                                       normalize-protection-js)))
