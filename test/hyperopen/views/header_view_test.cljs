...
(deftest header-renders-trading-settings-shell-when-open-test
  (let [view (header-view/header-view {:wallet {:connected? false
    ...
    (is (contains? panel-classes "w-[336px]"))
    (is (contains? panel-classes "rounded-[16px]"))
    (is (contains? panel-classes "bg-[#232a30]"))
    (is (contains? sheet-classes "bg-[#232a30]"))
    (is (contains? session-section-classes "rounded-[14px]"))
    (is (contains? session-section-classes "bg-[#252c32]"))
