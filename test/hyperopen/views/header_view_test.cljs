...
(deftest header-renders-trading-settings-shell-when-open-test
  (let [view (header-view/header-view {:wallet {:connected? false
        ...
        confirm-market-orders-row (find-node-by-role view "trading-settings-confirm-market-orders-row")
        sound-on-fill-row (find-node-by-role view "trading-settings-sound-on-fill-row")
        ...
        confirm-market-orders-icon (find-node-by-role view "trading-settings-confirm-market-orders-row-icon")
    ...
    (is (contains? panel-classes "ts-pop"))
    (is (contains? panel-classes "fixed"))
    (is (contains? panel-classes "w-[min(400px,calc(100vw-32px))]"))
    (is (contains? session-section-classes "ts-group"))
    (is (= "top right" (:transform-origin panel-style)))
    (is (= "translateY(8px) scale(0.98)" (:transform panel-mounting)))
    ...
    (is (some? confirm-market-orders-row))
    (is (some? sound-on-fill-row))
    (is (some? confirm-open-orders-icon))
    (is (some? confirm-close-position-icon))
    (is (some? confirm-market-orders-icon))
    (is (some? storage-icon))
    (is (some? storage-row))
    (is (some? fill-alerts-row))
    (is (some? footer-note))
    (is (some? (find-node #(contains? (class-token-set %) "ts-pop-icon") panel)))
    ...
    (is (contains? all-text "These settings live on this device only."))
    (is (not (contains? all-text ", to open · esc to close")))
    (is (nil? (find-node #(contains? (class-token-set %) "ts-pop-sub") panel)))
    (is (not (contains? all-text "Applies only to this browser on this device.")))
...
(deftest header-renders-remembered-session-default-when-storage-mode-is-missing-test
  (let [view (header-view/header-view {:wallet {:connected? false}
                                       ...
                                       :trading-settings {:fill-alerts-enabled? true}})
        storage-row (find-node-by-role view "trading-settings-storage-mode-row")
        storage-toggle (find-node #(and (vector? %)
                                        (= "switch" (get-in % [1 :role])))
                                  storage-row)
        all-text (set (collect-strings view))]
    (is (contains? all-text "Remember session"))
