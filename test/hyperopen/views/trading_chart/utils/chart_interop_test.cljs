...
(deftest volume-indicator-overlay-sync-renders-controls-and-crosshair-updates-test
  (let [document (make-fake-document)
    ...
    (let [state (@#'hyperopen.views.trading-chart.utils.chart-interop.volume-indicator-overlay/overlay-state
          ...
          remove-button (find-dom-node root
                                       #(= "Remove volume indicator"
                                           (aget ^js % "aria-label")))
          settings-button (find-dom-node root
      ...
      (dispatch-dom-event! panel "mouseenter")
      (is (= "inline-flex" (.-display (.-style controls))))
      (dispatch-dom-event! panel "mouseleave")
      (is (= "none" (.-display (.-style controls))))
      (dispatch-dom-event! panel "focusin")
      (is (= "inline-flex" (.-display (.-style controls))))
      ...
      (click-dom-node! settings-button)
      (click-dom-node! remove-button)
      (is (= 1 @remove-calls*)))))
