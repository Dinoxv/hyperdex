    ...
    (with-viewport-width
      ...
      (fn []
        (let [branch-node (render-visible-branch
          ...
          (is (= 1 @mobile-calls))
          (is (= 0 @desktop-calls))
          (is (some? (find-node-by-role branch-node "mobile-branch")))
          (is (nil? (find-node-by-role branch-node "desktop-branch"))))))
    ...
    (with-viewport-width
      ...
      (fn []
        (let [branch-node (render-visible-branch
          ...
          (is (= 0 @mobile-calls))
          (is (= 1 @desktop-calls))
          (is (some? (find-node-by-role branch-node "desktop-branch")))
          (is (nil? (find-node-by-role branch-node "mobile-branch"))))))))
  ...
  (with-viewport-width
    ...
    (fn []
      (let [view-node (row/select-asset-row {:visible-dropdown nil})
        ...
        (is (not (contains-class? view-node row/active-asset-grid-template)))
  ...
  (with-viewport-width
    ...
    (fn []
      (let [view-node (row/select-asset-row {:visible-dropdown nil})
            strings (set (collect-strings view-node))]
        (is (contains? strings "Select Asset"))
        (is (contains-class? view-node row/active-asset-grid-template))
