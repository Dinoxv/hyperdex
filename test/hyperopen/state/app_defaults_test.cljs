...
(deftest default-app-state-preserves-injected-defaults-and-core-shape-test
  (let [websocket-health {:transport {:state :connected}}
    ...
    (is (= false (get-in state [:account-context :shadow-mode :active?])))
    (is (nil? (get-in state [:account-context :shadow-mode :address])))
    (is (= false (get-in state [:account-context :shadow-ui :modal-open?])))
    (is (nil? (get-in state [:account-context :shadow-ui :anchor])))
    (is (= "" (get-in state [:account-context :shadow-ui :search])))
    (is (= "" (get-in state [:account-context :shadow-ui :label])))
    (is (nil? (get-in state [:account-context :shadow-ui :editing-watchlist-address])))
    (is (= "" (get-in state [:account-context :shadow-ui :last-search])))
    (is (nil? (get-in state [:account-context :shadow-ui :search-error])))
