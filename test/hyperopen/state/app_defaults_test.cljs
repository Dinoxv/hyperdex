...
(deftest default-app-state-preserves-injected-defaults-and-core-shape-test
  (let [websocket-health {:transport {:state :connected}}
    ...
    (is (= "" (get-in state [:leaderboard-ui :query])))
    (is (= :month (get-in state [:leaderboard-ui :timeframe])))
    (is (= :pnl (get-in state [:leaderboard-ui :sort :column])))
    (is (= :desc (get-in state [:leaderboard-ui :sort :direction])))
    (is (= 1 (get-in state [:leaderboard-ui :page])))
    (is (= 10 (get-in state [:leaderboard-ui :page-size])))
    (is (= [5 10 25 50] (get-in state [:leaderboard-ui :page-size-options])))
    (is (= false (get-in state [:leaderboard-ui :page-size-dropdown-open?])))
    ...
    (is (= false (get-in state [:staking-ui :validator-show-all?])))
    ...
    (is (= "" (get-in state [:staking-ui :undelegate-amount])))
    (is (nil? (get-in state [:staking-ui :form-error])))
    (is (= false (get-in state [:staking-ui :submitting :deposit?])))
    (is (= false (get-in state [:staking-ui :submitting :withdraw?])))
    (is (= false (get-in state [:staking-ui :submitting :delegate?])))
    (is (= false (get-in state [:staking-ui :submitting :undelegate?])))
    (is (= [] (get-in state [:staking :validator-summaries])))
    (is (nil? (get-in state [:staking :delegator-summary])))
