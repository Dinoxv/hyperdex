...
(deftest request-hyperunit-operations-uses-direct-base-url-candidate-test
  (async done
    (let [calls (atom [])]
      (-> (@#'hyperopen.funding.effects/with-hyperunit-base-url-fallbacks!
          ...
          (.then (fn [resp]
                   (is (= ["https://api.hyperunit.xyz"] @calls))
                   (is (= "op-1"
...
(deftest funding-effect-unit_and_error_helpers_cover_numeric_and_message_fallbacks_test
...
(deftest funding-effect-config_resolution_prefers_action_wallet_and_default_branches_test
...
(deftest funding-effect_route_and_rpc_wrappers_pass_expected_options_test
  (async done
